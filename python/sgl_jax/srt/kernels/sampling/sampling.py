...
from sgl_jax.srt.kernels.sampling.top_p_and_sample import top_p_and_sample, _top_p_and_sample
...
def topk_topp_and_sample_shmap(
  ...
  """Combined top-k, top-p filtering, and sampling for vLLM inference using shard_map.
  ...
  Returns:
    Sampled token indices.
  """
  batch_axis, vocab_axis = ("data", "tensor")
  ...
  def shmap_topp_and_sample(topk_logits, topk_idxs, rng_key, top_p, temperature, seeds, positions):
  ...
  # Determine specs based on whether seeds and positions are provided
  seeds_arg = seeds if seeds is not None else jnp.zeros((logits.shape[0],), dtype=jnp.int32)
  positions_arg = positions if positions is not None else jnp.zeros((logits.shape[0],), dtype=jnp.int32)
  ...
  return jax.shard_map(
    ...
    in_specs=(
      ...
      P(batch_axis),  # seeds
      P(batch_axis),  # positions
    ),
    out_specs=P(batch_axis),  # output tokens
    check_vma=False,
  )(topk_logits, topk_idxs, rng_key, tpu_sampling_metadata.top_p, tpu_sampling_metadata.temperature, seeds_arg, positions_arg)
